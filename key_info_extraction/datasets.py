    ...
    def connect(self, bboxes, imgw, imgh):
        ...
        for src_idx, src_row in enumerate(bboxes):
            ...
            for dest_idx, dest_row in enumerate(bboxes):
                if dest_idx == src_idx:
                    continue
                dest_row["x_min"], dest_row["y_min"], dest_row["x_max"], dest_row["y_max"] = dest_row["crop"][1][0], dest_row["crop"][1][1], dest_row["crop"][0][0], dest_row["crop"][0][1]
            ...
            if neighbor_hozi_left:
                nei = max(neighbor_hozi_left, key=lambda x: bboxes[x]['x_max'] - src_center_x)
                neighbors.append(nei)
                G.add_edge(src_idx, nei)
            if neighbor_hozi_right:
                nei = min(neighbor_hozi_right,
                          key=lambda x: src_center_x - bboxes[x]['x_min'])
                neighbors.append(nei)
                G.add_edge(src_idx, nei)
            ...
            if neighbor_vert_top:
                nei = min(neighbor_vert_top, key=lambda x: bboxes[x]['y_max'])
                neighbors.append(nei)
                G.add_edge(src_idx, nei)
